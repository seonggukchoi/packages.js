...
컴패니언 플러그인 [`@seonggukchoi/opencode-claude-code-plugin`](https://www.npmjs.com/package/@seonggukchoi/opencode-claude-code-plugin)을 함께 등록해야 합니다. 없으면 세션 이어하기와 제목 세션 격리가 동작하지 않습니다.
...
- **스트리밍 전용.** 스트리밍 응답만 지원되며, 비스트리밍 호출은 사용할 수 없습니다.
- **도구는 OpenCode를 통해 라우팅.** CLI가 직접 도구를 실행하지 않으며, 모든 도구 호출은 파싱 후 OpenCode에 전달되어 실행됩니다.
- **Claude Code의 `CLAUDE.md`는 로딩하지 않음.** Claude Code 자체의 프로젝트 지시문 대신 OpenCode가 자체 컨텍스트(예: `AGENTS.md`)를 제공합니다.
- **권한 검사 우회.** `--dangerously-skip-permissions`로 실행됩니다. 모든 도구 실행은 자체 권한 모델을 가진 OpenCode에 위임됩니다.

## 알려진 제약 사항

- **컴패니언 플러그인 필수.** [`@seonggukchoi/opencode-claude-code-plugin`](https://www.npmjs.com/package/@seonggukchoi/opencode-claude-code-plugin)을 이 프로바이더와 함께 등록해야 합니다. 없으면 세션 이어하기와 제목 세션 격리가 정상 동작하지 않습니다.
- 컨텍스트 윈도우 크기는 CLI가 결정; 1M 전체를 사용하려면 `[1m]` 필요
- **모델 교차 사용 시 대화 연속성 없음.** 일반 API 프로바이더는 매 요청마다 전체 대화 히스토리를 전송하지만, 이 프로바이더는 상태를 유지하는 CLI 세션을 이어서 사용합니다. 세션 중간에 다른 모델(예: GPT)로 전환한 뒤 다시 돌아오면, Claude는 다른 모델과 주고받은 메시지를 인식하지 못합니다. AI SDK가 메시지에 프로바이더 메타데이터를 전달하지 않아, 어떤 메시지가 Claude의 것이고 어떤 것이 다른 모델의 것인지 구분할 수 없기 때문입니다.
- **Undo가 Claude Code CLI 세션을 되감지 않음.** OpenCode의 undo(`/undo`)는 OpenCode 자체의 히스토리에서 메시지를 제거하고 Git을 통해 파일 변경을 되돌리지만, Claude Code CLI 세션을 되감을 수는 없습니다. CLI는 `/rewind` 명령에 대한 프로그래밍 API를 제공하지 않으며, 인터랙티브 터미널 모드에서만 사용할 수 있습니다. Undo 이후에도 CLI 세션은 내부적으로 되돌려진 컨텍스트를 유지하므로, OpenCode와 Claude Code CLI 간에 히스토리 불일치가 발생할 수 있습니다.
