...
- **세션 이름** — 세션에 사용자 지정 이름이 있으면(예: `/rename`으로 설정) 해당 이름을 사용합니다.
- **작업 디렉토리** — 그렇지 않으면 현재 디렉토리 이름을 폴백으로 사용합니다.
- **위임된 에이전트** — 서브에이전트 이벤트(`subagentStarted` / `subagentCompleted`)에서는 부모 세션 컨텍스트에 에이전트 종류가 `세션(에이전트종류)` 형식으로 덧붙습니다(예: `my-project(Explore)`). Agent/Task 도구나 워크플로우로 위임된 에이전트는 부모 세션 아래에서 실행되므로, 단순 해시 대신 어느 세션에서 어떤 위임 에이전트가 보낸 알림인지 분명하게 구분할 수 있습니다.
