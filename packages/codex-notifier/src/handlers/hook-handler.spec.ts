...
describe('buildNotification', () => {
  ...
  it('builds sessionInterrupted notification', () => {
    const result = buildNotification('sessionInterrupted', messages, {});

    expect(result.title).toBe('⏹️ Codex');
    expect(result.message).toBe('Session interrupted.');
    expect(result.sound).toBe('Funk');
  });
  ...
  it('builds decisionNeeded notification using decisionRequired as question', () => {
    const hookData: HookData = { tool_name: 'request_user_input', tool_input: { questions: [] } };
    const result = buildNotification('decisionNeeded', messages, hookData);
  ...
  it('builds subagentStarted notification with agent_type from hook data', () => {
    const hookData: HookData = { agent_type: 'explorer' };
    const result = buildNotification('subagentStarted', messages, hookData);

    expect(result.title).toBe('🤖 Codex');
    expect(result.message).toBe('Subagent started: explorer');
