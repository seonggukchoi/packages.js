...
describe('ensureIconCache', () => {
  it('creates cache directory if it does not exist', () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith('/mock-home/.codex/plugins/codex-notifier/icons', { recursive: true });
  });
  ...
  it('skips if icns file does not exist', () => {
    mockedExistsSync.mockImplementation((path: unknown) => {
      ...
      if (pathStr === '/mock-home/.codex/plugins/codex-notifier/icons') {
        return true;
      }
...
describe('getPngIcon', () => {
  it('returns path when file exists', () => {
    ...
    expect(result).toBe('/mock-home/.codex/plugins/codex-notifier/icons/iTerm2.png');
  });
