...
import { resolvePluginDir } from './paths.js';

const CACHE_DIR_NAME = 'icons';
...
function getCacheDir(): string {
  return join(resolvePluginDir(), CACHE_DIR_NAME);
}

function getPngPath(app: string): string {
  return join(getCacheDir(), `${app.replace(/[^a-zA-Z0-9]/g, '_')}.png`);
}
